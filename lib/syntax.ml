...
(*     match d with *)
(*     | NameTy s -> pp_sym s *)
(*     | RecordTy fs -> braces @@ separate comma_space @@ List.map pp_field fs *)
...
(*     | Call (s, e) -> *)
(*        let a = separate comma_space @@ List.map pp e in *)
(*        pp_sym s ^^ parens a *)
(*     | Seq b -> *)
(*        let bs = List.map pp b in *)
(*        parens @@ separate (string "; ") bs *)
(*     | Record (name, e) -> *)
(*        let b (s,x) = pp_sym s ^^ string "=" ^^ pp x in *)
(*        let a = separate comma_space @@ List.map b e in *)
...
(*     | Let (d, b) -> *)
(*        let e = separate (string ";" ^^ newline) @@ List.map pp b.L.item in *)
(*        string "let" ^^ newline ^^ *)
...
(*       let f = d.L.item in *)
(*       let p = separate comma_space @@ List.map pp_field f.params in *)
(*       string "function" ^^ pp_sym f.fun_name ^^ parens p ^^ *)
(*         (Option.bind f.result_typ  (fun x -> Some (string ":" ^^ pp_sym x)) |> Option.value ~default:(string "")) ^^ *)
(*           string "=" ^^ newline ^^ *)
(*             indent (pp f.body) *)

(*   and pp_dec (d: dec) : SmartPrint.t = *)
(*     match d with *)
(*     | FunctionDec a -> separate newline @@ List.map pp_fun_dec a *)
(*     | VarDec a -> pp_var_dec a *)
(*     | TypeDec td -> separate newline @@ List.map pp_typ_dec td *)

(*   and pp_decs (d : dec list) : SmartPrint.t = *)
(*     separate (newline) @@ List.map pp_dec d *)

(*   and pp_var_dec (d : vardec L.loc) : SmartPrint.t = *)
(*     let a = Option.bind d.L.item.var_typ (fun x -> Some (string ":" ^-^ pp_sym x)) *)
